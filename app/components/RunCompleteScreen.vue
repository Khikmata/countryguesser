      ...
      <Transition name="fail-card" appear>
        ...
        >
          <template v-if="snapshot.mode === 'one-life'">
            ...
            <h2 class="mt-2 text-center text-3xl font-extrabold tracking-tight text-neutral-900 dark:text-white">
              You cleared every flag
            </h2>
            <p class="mt-4 text-center text-base leading-relaxed text-neutral-600 dark:text-neutral-300">
              {{ snapshot.message }}
            </p>
            <div class="mt-8 rounded-2xl bg-emerald-500/10 py-5 text-center dark:bg-emerald-500/15">
              <p class="text-xs font-medium uppercase tracking-wide text-emerald-800 dark:text-emerald-200">
                Total score
              </p>
              <p class="mt-1 text-4xl font-bold tabular-nums text-emerald-700 dark:text-emerald-300">
                {{ snapshot.score }}
              </p>
          ...
          <template v-else>
            ...
            <dl class="mt-6 space-y-3 text-center">
              <div class="rounded-2xl bg-violet-500/10 py-4 dark:bg-violet-500/15">
                <dt class="text-xs font-medium uppercase tracking-wide text-violet-800 dark:text-violet-200">
                  Final score
                </dt>
                <dd class="mt-1 text-3xl font-bold tabular-nums text-violet-900 dark:text-violet-100">
                  {{ snapshot.score }}
                </dd>
              </div>
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="rounded-2xl bg-amber-500/10 py-4 dark:bg-amber-500/15">
                  <dt class="text-xs font-medium uppercase tracking-wide text-amber-900 dark:text-amber-100">
                    Best streak (this run)
                  </dt>
                  <dd class="mt-1 text-2xl font-bold tabular-nums text-amber-950 dark:text-amber-50">
            ...
            <div class="mt-6">
              ...
              >
                ...
                >
                  <span class="font-medium text-neutral-800 dark:text-neutral-100">{{ row.name }}</span>
                  <span
          ...
          <div class="mt-8 flex flex-col gap-3">
            <UButton block size="xl" class="font-bold" @click="emit('play-again')"> Play again </UButton>
            <UButton block size="lg" variant="soft" color="neutral" class="font-semibold" @click="emit('back-leaderboard')"> To leaderboards </UButton>
            <UButton block size="lg" variant="soft" color="neutral" class="font-semibold" @click="emit('back-menu')">
              Mode menu
            </UButton>
          </div>
        </div>
