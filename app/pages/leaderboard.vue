  ...
  >
    <main class="mx-auto max-w-lg px-5 pb-24 pt-12 md:max-w-xl md:pt-16">
      ...
      <p class="mt-4 text-sm text-neutral-600 dark:text-neutral-400">
        ...
        <template v-else>
          Marathon keeps your run going after a miss. New rows appear only when you beat your previous best total
          score. Stored on this device only.
        </template>
      </p>
        ...
        >
          ...
          >
            <span class="flex min-w-0 items-center gap-3">
              ...
              <span class="truncate font-semibold tabular-nums text-emerald-700 dark:text-emerald-400">
          ...
          <li v-if="rows.length === 0" class="px-4 py-12 text-center text-neutral-500 dark:text-neutral-400">
            <template v-if="activeMode === 'one-life'">No One life scores yet.</template>
            <template v-else>No Marathon highs yet — keep playing to set a best score.</template>
          </li>
        </ul>
        <template #fallback>
